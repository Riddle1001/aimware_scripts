-- I put different scripts in here
function gui._Custom(ref, varname, name, x, y, w, h, paint, custom_vars)
	...
	local GuiObject = {
		...
		SetChild = function(self)
 
			self._child = true
			print("Hi", varname, self:IsChild())
		end,
...
function gui._GroupBox(ref, x, y, w, h)
	local function paint(x, y, x2, y2, active, self, width, height)
		...
		draw.FilledRect(x, y, x2, y2)
		draw.Color(150,0,0)
		draw.FilledRect(x, y, x2, y2 - (h / 1.15))
	end
	...
	custom.header_text = gui.ColoredText(test_tab, "Lua loader V2 | Last UPDATE | Made by chicken",5, h / 29, {font = font})
...
local gb = gui._GroupBox(test_tab, 15, 15, 605, 200)
 
-- local img_data = {common.DecodeJPEG(http.Get("https://cdn.cloudflare.steamstatic.com/steamcommunity/public/images/avatars/e5/e53474dbea973d880cb24e5d7247ad77fbb68721_full.jpg"))}
